...
mpu6050::mpu6050(hal::steady_clock& p_clock, hal::soft::bit_bang_i2c& p_i2c)
  ...
  , m_i2c(p_i2c)
{
  std::array<hal::byte, 2> power_settings = { addresses::power_address, 0 };
...
void mpu6050::read_accel()
{
  std::array<hal::byte, 1> adresses_write_to = {
    addresses::accel_xout_first_half
  };
  std::array<hal::byte, 6> buffer;
  hal::write_then_read(
    m_i2c, addresses::device_address, adresses_write_to, buffer);
...
{
  mpu6050::read_accel();
  float roll = (std::atan(accel.accely / std::sqrt(std::pow(accel.accelx, 2) +
